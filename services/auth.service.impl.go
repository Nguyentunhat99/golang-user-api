...
type AuthServiceImpl struct { //cấu trúc Bộ sưu tập MongoDB và giao diện Ngữ cảnh của gói ngữ cảnh
	collection *mongo.Collection
	ctx        context.Context
}

func NewAuthService(collection *mongo.Collection, ctx context.Context) AuthService {
	return &AuthServiceImpl{collection, ctx} //trả ve dia chi cua AuthServiceIpml
}

//ham khoi tao cau truc bo suu tap vs giao dien ngu canh
//cung triern khai interface AuthService
...
func (uc *AuthServiceImpl) SignUpUser(user *models.SignUpInput) (*models.DBResponse, error) {
	...
	res, err := uc.collection.InsertOne(uc.ctx, &user) //day vao csdl

	if err != nil {
		if er, ok := err.(mongo.WriteException); ok && er.WriteErrors[0].Code == 11000 { //kiem tra
